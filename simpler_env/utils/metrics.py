...
def print_all_kruskal_results(
    ...
    for i in range(sim.shape[0]):
            try:
                ...
                if str(err) == "All numbers are identical in kruskal":
...
def kruskal_wallis(
    ...
    sim, real: shape [n_ckpt, n_trials]
        The trial-by-trial success indicator of each checkpoint
        (within each checkpoint, the ordering doesn't matter)
    Calculate the Kruskal-Wallis test for each checkpoint 
    """
    from scipy.stats import kruskal
    sim, real = np.array(sim), np.array(real)
    results = []
    for i in range(sim.shape[0]):
            try:
                results.append(kruskal(sim[i], real[i]))
            except ValueError as err:
                if str(err) == "All numbers are identical in kruskal":
                    # handle the case where all values are identical
                    results.append(None)
...
def get_dir_stats2(
    dir_name: str,
    extra_pattern_require: Optional[Sequence[str]] = None,
    succ_fail_pattern: Sequence[str] = ["success", "failure"],
):
    # Use an empty list if no extra patterns are provided.
    if extra_pattern_require is None:
    ...
    files = glob.glob(f"{dir_name}/**/*.mp4", recursive=True)
    
    results = []
    for file in files:
        # Only proceed if the file name contains all required patterns.
        if not all(pattern in file for pattern in extra_pattern_require):
            continue
        # Get the file name without the extension.
        stem = Path(file).stem
        # Append 1 if the success pattern is found, 0 if the failure pattern is found.
        if succ_fail_pattern[0] in stem:
            results.append(1)
        if succ_fail_pattern[1] in stem:
            results.append(0)
    return sum(results)