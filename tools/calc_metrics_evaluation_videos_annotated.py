...
def calc_drawer_stats(root_result_dir, mode="both"):
    # Print header indicating the start of drawer results analysis.
    print("***Drawer results***")
    
    # Define real-world success rates for 'open' and 'close' drawer tasks across different checkpoints.
    # These are hardcoded values that should be updated when using a new checkpoint.
    drawer_real_success = {
        "open": {
          #  "rt-2-x": 0.333,
    ...
    # Get the list of checkpoint alias keys from the 'open' task's dictionary.
    ckpt_alias_keys = list(drawer_real_success["open"].keys())
    
    #ckpt_alias_keys = ["rt-1-converged", "rt-1-15pct", "rt-1-x", "rt-1-begin",]

    # Map drawer tasks ('open' and 'close') to a list of specific simulation task identifiers.
    ...
    # Number of drawer tasks (here, 2 tasks: 'open' and 'close').
    n_tasks = len(drawer_task_map_dict)
    
    # Number of trials per checkpoint per task in the simulation.
    n_trials_per_ckpt_per_task = (
        36  # number of trials per checkpoint for all open / all close tasks; update if it is different
    )
    if mode in ["both", "variant_aggregation"]:

        ########################################
        #          VARIANT AGGREGATION
        ########################################
        
        # Hardcoded base variant pattern for simulation result directories.
        base_variants = [
        ...
        # Define additional variant patterns for background, lighting, and table textures.
        background_variants = [
    ...
    # Define extra filename pattern requirements for simulation variant success extraction.
        extra_pattern_require_sim_variants = ["rgb_overlay_None"]

        # Initialize a dictionary to store simulation success rates for each drawer task and checkpoint.
        drawer_sim_variant_success = {k1: {k2: [] for k2 in ckpt_alias_keys} for k1 in drawer_task_map_dict.keys()}
    
        # Iterate over each drawer task type ('open' and 'close').
        for drawer_task in drawer_task_map_dict.keys():
            # Iterate over each checkpoint alias.
            for ckpt_alias in ckpt_alias_keys:
                # Iterate over each specific simulation task for the current drawer task.
                for specific_task in drawer_task_map_dict[drawer_task]:
                    # Iterate over each variant pattern from base, background, lighting, and table texture lists.
                    for variant in base_variants + background_variants + lighting_variants + table_texture_variants:
                        # Format the variant string with the specific task name.
                        variant = variant.format(specific_task)
                        # Construct the complete path by combining the root directory, checkpoint mapping, and the variant path.
                        variant = f"{root_result_dir}/{CKPT_MAPPING[ckpt_alias]}/{variant}"
                        # Retrieve statistics from the directory and compute the average simulation success.
                        avg_sim_success = np.mean(
                            get_dir_stats(
                                variant,
                                extra_pattern_require=extra_pattern_require_sim_variants,
                        ...
                        # If the computed average is NaN, print a warning with the directory path.
                        if np.isnan(avg_sim_success):
                            print(f"WARNING: avg_sim_success is nan for {variant}")
                        # Append the computed success rate to the list for the corresponding drawer task and checkpoint.
                        drawer_sim_variant_success[drawer_task][ckpt_alias].append(avg_sim_success)
                # Compute the mean simulation success for the current drawer task and checkpoint by averaging over all variants.
                drawer_sim_variant_success[drawer_task][ckpt_alias] = np.mean(
                    drawer_sim_variant_success[drawer_task][ckpt_alias]
                )

        print("-" * 20)
        # For each drawer task, print the simulation variant average success, real success,
        # and calculate additional metrics (MMRV and Pearson correlation) comparing simulation to real success.
        for drawer_task in drawer_task_map_dict.keys():
            print(
        ...
        # Aggregate the average simulation and real success results across tasks for each checkpoint.
        avg_sim_variant_results = []
        avg_real_results = []
        for ckpt_alias in ckpt_alias_keys:
            avg_sim_variant_results.append([])
            avg_real_results.append([])
            # For each drawer task, collect the simulation and real success rates.
            for drawer_task in drawer_task_map_dict.keys():
            ...
            # Compute the mean values across tasks for the current checkpoint.
    ...
    if mode in ["both", "visual_matching"]:

    ########################################
    #          VISUAL MATCHING
    ########################################

        # Build a list of visual matching variant patterns using different URDF versions.
        base_visual_matching_variants = []
        for visual_matching_urdf_version in [
            "None",
            "recolor_tabletop_visual_matching_1",
        ...
        # Initialize a dictionary for visual matching simulation success rates, similar to the simulation variant one.
        ...
        # Iterate over open and close 2 
        for drawer_task in drawer_task_map_dict.keys():
            # Iterate over each 5 policy
            eval_means = []
            for ckpt_alias in ckpt_alias_keys:
                eval_results = []
                # Iterate over top, middle, bottom 3
                for specific_task in drawer_task_map_dict[drawer_task]: 
                    # Iterate over urdf versions
                    for variant in base_visual_matching_variants: #4
                        variant = variant.format(specific_task)
                        variant = f"{root_result_dir}/{CKPT_MAPPING[ckpt_alias]}/{variant}"
                        # Compute the average simulation success for visual matching using required file name patterns.
                        arr = get_dir_stats(
                                variant,
                                extra_pattern_require=extra_pattern_require_visual_matching,
                            ) 
                        eval_results.extend(arr)
                        
                        # Mean over 9 different robot locations
                        avg_sim_success = np.mean(arr)
                        # Print a warning if the average simulation success is NaN.
                        if np.isnan(avg_sim_success):
                            print(f"WARNING: avg_sim_success is nan for {variant}")
                        # Append the visual matching success result to the corresponding list.
                        drawer_sim_visual_matching_success[drawer_task][ckpt_alias].append(avg_sim_success)
                # Compute average success for each robot arm variant by splitting the results list.
                tmp_variant_avg_each_robot_arm = []
                # Mean over the 3 different drawer (top, middle, bottom) for each urdf version
                ...
                # Print the computed visual matching success for each robot arm variant.
                print(f"Drawer task {drawer_task}, ckpt {ckpt_alias} all robot arm visual matching success: {tmp_variant_avg_each_robot_arm}")
                
                print("Before", drawer_sim_visual_matching_success[drawer_task][ckpt_alias])
                # Mean over 4 different urdf versions * 3 different drawer (top, middle, bottom)
                ...
                print("After ", drawer_sim_visual_matching_success[drawer_task][ckpt_alias])
                print(len(eval_results))
                eval_means.append(np.mean(eval_results))
                trials = construct_unordered_trial_results(108, eval_means)
                reconstructed_means = np.mean(trials, axis=1)
                print(eval_means)
                print(type(eval_means[0]))
                print(reconstructed_means)
                print(type(reconstructed_means[0]))
                
        print(60 * "-")
        
        def create_eval_df(data, eval_label):
            df = pd.DataFrame(data).reset_index()
            df["Average"] = (df["open"] + df["close"]) / 2
            df.insert(0, 'Eval Setup', eval_label)
            return df

        real_df = create_eval_df(drawer_real_success, 'Real')
        sim_df = create_eval_df(drawer_sim_visual_matching_success, 'Sim')
        
        # compute MMRV and Pearson correlation for each task
        tasks = ['open', 'close', 'Average']

        get_eval_result_lists = lambda task: (sim_df[task].tolist(), real_df[task].tolist())
        
        trial_results = construct_unordered_trial_results(n_trials_per_ckpt_per_task, sim_df["open"].tolist())
        print("Trial Results: ", trial_results)

        print(np.mean(trial_results, axis=1)) 
        mmrv_results = {task: mean_maximum_rank_violation(*get_eval_result_lists(task))
                        for task in tasks}
        
        pearson_results = {task: pearson_correlation(*get_eval_result_lists(task))
                        for task in tasks}
        
        metric_rows = pd.DataFrame([
            {**{'Eval Setup': 'Metric', 'index': 'MMRV'}, **mmrv_results},
            {**{'Eval Setup': 'Metric', 'index': 'Pearson'}, **pearson_results}
        ])

        # create a dataframe with all the results
        df = pd.concat([real_df, sim_df, metric_rows], ignore_index=True)
        df = df.rename(columns={"index": "Policy", "open": "Open", "close": "Close"})
        print(df.to_string(index=False))

        
        sim = construct_unordered_trial_results(108, sim_df["open"].tolist())
        real = construct_unordered_trial_results(27, real_df["open"].tolist())
        kruskal_results = [
            kruskal(sim[i], real[i]).pvalue if np.mean(sim[i]) != np.mean(real[i]) else 1
        ...
        # For each drawer task, print the visual matching simulation success,
        # compare with real success, and calculate additional metrics.
        for drawer_task in drawer_task_map_dict.keys():
            # Compute and print Kruskal-Wallis test results to compare simulation and real success distributions.
            print("HERE ",list(drawer_sim_visual_matching_success[drawer_task].values()))
            print_all_kruskal_results(
                construct_unordered_trial_results(
                    108,
        ...
        # Aggregate the average visual matching simulation and real success results for each checkpoint across tasks.
        ...
        for ckpt_alias in ckpt_alias_keys:
            ...
            for drawer_task in drawer_task_map_dict.keys():
                ...
                avg_real_results[-1].append(drawer_real_success[drawer_task][ckpt_alias])
            # Compute the mean value for each checkpoint.
        ...
        print(n_tasks)
        # Print Kruskal-Wallis test results for the aggregated visual matching data.
        print_all_kruskal_results(
            construct_unordered_trial_results(216, avg_sim_visual_matching_results),
            construct_unordered_trial_results(54, avg_real_results),
            "avg kruskal:",
        )

        print("-" * 20)

        # Final separator and spacing at the end of the output.
        print("*" * 20)
        for _ in range(3):
            print()
...
parser.add_argument("--mode", type=str, default="both", help="mode: variant_aggregation, visual_matching, or both")
...
if args.task == "pick_coke_can":
    ...
    if args.mode == "visual_matching":
        calc_drawer_stats(args.log_dir_root, mode="visual_matching")
    elif args.mode == "variant_aggregation":
        calc_drawer_stats(args.log_dir_root, mode="variant_aggregation")
    else:
        calc_drawer_stats(args.log_dir_root, mode="both")
